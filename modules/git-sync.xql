...
let $collection := xs:anyURI('/db/apps/SAI') 
let $file-name := $file-info//*:pair[@name="name"]/text()
let $resource-path := substring-before(replace($modified,'SAI/',''),$file-name)
             ...
             if(xmldb:collection-available($collection-uri)) then 
                 <response status="okay">
                    ...
                    (
                       ...
                       sm:chgrp(xs:anyURI(concat($collection-uri,$file-name)), 'SAI')
                    )
                    }
                    </message>
                 </response>
