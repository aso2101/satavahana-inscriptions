...
declare function model:apply($config as map(*), $input as node()*) {
    ...
    epub:block($config, ., ("tei-div3"), .)
                ...
                case element(figDesc) return
                    epub:block($config, ., ("tei-figDesc", "text-center"), .)
                case element(figure) return
                    if (head) then
                        html:figure($config, ., ("tei-figure1", "figure"), *[not(self::head)], head/node())
                    else
                        epub:block($config, ., ("tei-figure2", "figure", "text-center"), .)
                    ...
                    if (ancestor::teiHeader and parent::cit) then
                        ...
                        if (@rend='double') then
                            ...
                            if (ancestor::p or ancestor::note or ancestor::desc) then
                                ...
                                epub:block($config, ., css:get-rendition(., ("tei-quote4")), .)
    ...
    if ((@level='j' or @level='m') and ancestor::biblStruct) then
        ...
        html:inline($config, ., ("tei-title12"), .)),
                ...
                case element(biblStruct) return
                    epub:block($config, ., ("tei-biblStruct1", "bibl-citation"), (
        ...
        (
            ...
            if (relatedItem[@type='reviewOf']) then
                (
                    ...
                    html:link($config, ., ("tei-biblStruct5"), let $ref := substring-after(./relatedItem/ref/@target,'#') return ancestor::listBibl/biblStruct[@xml:id=$ref]/*/title[@type='short']/text(),  '?tabs=no&amp;odd=' || request:get-parameter('odd', ()) || '?' || ./relatedItem/ref/@target),
                    if (following-sibling::*) then
    ...
    $config?apply($config, ./node()))
                    ...
                    if (ancestor::listPlace and not(ancestor::listPlace/ancestor::listPlace)) then
                        ...
                        if (ancestor::listPlace) then
                            (
                                ...
                                epub:block($config, ., ("tei-place9"), desc)
                            )
                ...
                case element(placeName) return
                    if (ancestor::div[@type] or ancestor::origPlace) then
                        html:link($config, ., ("tei-placeName1"), ., ())
                    else
