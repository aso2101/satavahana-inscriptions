                    ...
                    if (parent::bibl and @target and following-sibling::citedRange) then
                        ...
                        if (parent::bibl and @target) then
                            ...
                            if (not(parent::bibl) and not(text()) and @target[starts-with(.,'#')]) then
                                ...
                                if (not(text())) then
                                    latex:link($config, ., ("tei-ptr4"), @target, ())
                                else
                                    $config?apply($config, ./node())
                case element(ptr) return
                    (: No function found for behavior: refbibl :)
                    ...
                    if (ancestor::listApp) then
                        (
                            ...
                            if (@source and ancestor::listApp) then
                                ...
                                (),
                            if (starts-with(@resp,'eiad-part:')) then
                                latex:inline($config, ., ("tei-rdg3"), substring-after(@resp,'eiad-part'))
                            else
                                (),
                            if (starts-with(@resp,'eiad-part:')) then
                                latex:inline($config, ., ("tei-rdg4"), substring-after(@resp,'eiad-part'))
                            else
                                (),
                            if (starts-with(@resp,'#')) then
                                latex:link($config, ., ("tei-rdg5"), substring-after(@resp,'#'),  "?odd=" || request:get-parameter("odd", ()) || "&amp;view=" || request:get-parameter("view", ()) || "&amp;id=" || @resp )
                            else
                                (),
                            if (@rend) then
                                latex:inline($config, ., ("tei-rdg6"), @rend)
                            else
                                (),
                            if (not(following-sibling::*[1][local-name()='rdg'])) then
                                latex:inline($config, ., ("tei-rdg7", "period"), '.')
                            else
                                ()
                        )

                    else
                        latex:inline($config, ., ("tei-rdg8"), .)
