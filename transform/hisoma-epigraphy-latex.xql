...
module namespace model="http://www.tei-c.org/pm/models/hisoma-epigraphy/latex";
...
declare function model:apply($config as map(*), $input as node()*) {
    ...
    (
        latex:text($config, ., ("tei-author3"), surname),
        latex:text($config, ., ("tei-author4"), ', '),
        latex:text($config, ., ("tei-author5"), forename)
    )

else
    if (name) then
        latex:text($config, ., ("tei-author6"), name)
    else
        $config?apply($config, ./node()))
                        else
                            $config?apply($config, ./node())
                        ...
                        if (ancestor::biblStruct and @unit='volume') then
                            (
                                if (monogr/author or monogr/editor) then
                                    ...
                                    (),
                                if (@unit='volume') then
                                    latex:inline($config, ., ("tei-biblScope3"), .)
                                else
                                    (),
                                if (@unit='volume') then
                                    latex:text($config, ., ("tei-biblScope4"), ', ')
                                else
                                    ()
                            )
                ...
                case element(div) return
                    if (ancestor::TEI[@type='general-biblio']) then
                        ...
                        if (@type='bibliography') then
                            (
                                latex:heading($config, ., ("tei-div2"), 'Secondary bibliography'),
                                latex:section($config, ., ("tei-div3", "bibliography-secondary"), listBibl)
                            )

                        else
                            if (not((@type='textpart') or (@type='apparatus'))) then
                                (
                                    latex:heading($config, ., ("tei-div4", (@type)), concat(upper-case(substring(@type,1,1)),substring(@type,2))),
                                    latex:section($config, ., ("tei-div5", (@type)), .)
                                )

                            else
                                if (@type='apparatus') then
                                    (
                                        latex:section($config, ., ("tei-div6", (@type)), .)
                                    )

                                else
                                    if (@type='edition') then
                                        (
                                            latex:section($config, ., ("tei-div7", (@type)), .)
                                        )

                                    else
                                        if (@type='textpart') then
                                            latex:block($config, ., ("tei-div8"), .)
                                        else
    ...
    (
        latex:text($config, ., ("tei-editor3"), surname),
        latex:text($config, ., ("tei-editor4"), ', '),
        latex:text($config, ., ("tei-editor5"), forename)
    )

else
    if (name) then
        latex:text($config, ., ("tei-editor6"), name)
    else
        $config?apply($config, ./node()))
                        else
                            $config?apply($config, ./node())
    ...
    latex:text($config, ., ("tei-lb4"), ' ('),
    latex:inline($config, ., ("tei-lb5"), if (@n) then @n else count(preceding-sibling::lb) + 1),
    latex:text($config, ., ("tei-lb6"), ') ')
                ...
                case element(lg) return
                    if (@met or @n) then
                        ...
                        latex:block($config, ., ("tei-lg4", "block"), .)
                ...
                case element(fileDesc) return
                    if ($parameters?header='short') then
                        ...
                        if (ancestor::TEI[@type='general-biblio']) then
                            latex:paragraph($config, ., ("tei-fileDesc3"), sourceDesc)
                        else
                            $config?apply($config, ./node())
                ...
                case element(revisionDesc) return
                    if ($parameters?teiHeader-type='epidoc') then
                        ...
                        (
                            latex:block($config, ., ("tei-revisionDesc1"), concat('Last modified on: ',change[position()=last()]/@when))
                        )
                                ...
                                if ((@level='j' or @level='m')  and ancestor::biblStruct) then
                                    ...
                                    if (not(@level) and parent::bibl) then
                                        ...
                                        if (@type='short' and ancestor::biblStruct) then
                                            latex:inline($config, ., ("tei-title7", "vedette"), .)
                                        else
                ...
                case element(biblStruct) return
                    if (parent::listBibl) then
                        latex:listItem($config, ., ("tei-biblStruct1"), (
    latex:block($config, ., ("tei-biblStruct2"), (
    if (./@type) then
        latex:inline($config, ., ("tei-biblStruct3"), @type)
    else
        (),
    if (./@xml:id) then
        latex:inline($config, ., ("tei-biblStruct4"), @xml:id)
    else
        ()
)
),
    (
        latex:block($config, ., ("tei-biblStruct5"), if (.//title[@type='short']) then
    latex:inline($config, ., ("tei-biblStruct6"), .//title[@type='short'])
else
    (
        if (.//author/surname) then
            latex:text($config, ., ("tei-biblStruct7"), .//author/surname)
        else
            if (.//author/name) then
                latex:text($config, ., ("tei-biblStruct8"), .//author/name)
            else
                $config?apply($config, ./node()),
        latex:text($config, ., ("tei-biblStruct9"), ' '),
        latex:inline($config, ., ("tei-biblStruct10"), monogr/imprint/date)
    )
)
    )
,
    (
        if (@type='journalArticle' or @type='bookSection' or @type='encyclopediaArticle') then
            (
                latex:inline($config, ., ("tei-biblStruct11"), analytic/author),
                latex:text($config, ., ("tei-biblStruct12"), ', '),
                latex:inline($config, ., ("tei-biblStruct13"), analytic/title[@level='a']),
                latex:text($config, ., ("tei-biblStruct14"), ', '),
                if (@type='bookSection' or @type='encyclopediaArticle') then
                    (
                        if (@type='bookSection' or @type='encyclopediaArticle') then
                            latex:text($config, ., ("tei-biblStruct15"), 'in ')
                        else
                            (),
                        latex:inline($config, ., ("tei-biblStruct16"), monogr/title[@level='m']),
                        latex:text($config, ., ("tei-biblStruct17"), ', '),
                        if (monogr/author) then
                            latex:text($config, ., ("tei-biblStruct18"), 'by ')
                        else
                            (),
                        latex:inline($config, ., ("tei-biblStruct19"), monogr/author),
                        if (monogr/editor) then
                            latex:text($config, ., ("tei-biblStruct20"), 'edited by ')
                        else
                            (),
                        latex:inline($config, ., ("tei-biblStruct21"), monogr/editor)
                    )
                ...
                if (@type='journalArticle') then
                    (
                        latex:inline($config, ., ("tei-biblStruct22"), monogr/title[@level='j']),
                    ...
                    (),
                if (.//series) then
                    latex:inline($config, ., ("tei-biblStruct24"), series)
                else
                    (),
                if (.//monogr/imprint) then
                    latex:inline($config, ., ("tei-biblStruct25"), monogr/imprint)
                else
                    (),
                if (.//note) then
                    (
                        ...
                        if (.//imprint/note) then
                            latex:inline($config, ., ("tei-biblStruct27"), ./note)
                        else
                            (),
                        if (not(ends-with(.//note,'.'))) then
                            latex:inline($config, ., ("tei-biblStruct28"), '.')
                        else
                            ()
                    )
            ...
            if (@type='book') then
                (
                    latex:inline($config, ., ("tei-biblStruct29"), monogr/author),
                    latex:text($config, ., ("tei-biblStruct30"), ', '),
                    latex:inline($config, ., ("tei-biblStruct31"), monogr/title[@level='m']),
                    latex:text($config, ., ("tei-biblStruct32"), ', '),
                    if (monogr/imprint) then
                        (
                    ...
                    if (note) then
                        (
                            ...
                            if (not(ends-with(note,'.'))) then
                                latex:inline($config, ., ("tei-biblStruct35"), '.')
                            else
                                ()
                        )
                        ...
                        if (ancestor::biblStruct[@type='journalArticle']) then
                            (
                                if (biblScope[@unit='volume']) then
                                    latex:inline($config, ., ("tei-imprint7"), biblScope[@unit='volume'])
                                else
                                    (),
                                ...
                                if (../biblScope[@unit='page']) then
                                    latex:text($config, ., ("tei-imprint10"), ': ')
                                else
                                    (),
                                if (note) then
                                    ...
                                    (),
                                if (not(ancestor::biblStruct//note)) then
                                    latex:inline($config, ., ("tei-imprint12"), '.')
                                else
                                    ()
                            )
                            ...
                            if (ancestor::biblStruct[@type='encyclopediaArticle'] or ancestor::biblStruct[@type='bookSection']) then
                                (
                                    if (biblScope[@unit='volume']) then
                                        latex:inline($config, ., ("tei-imprint13"), biblScope[@unit='volume'])
                                    else
                                        (),
                                    if (following-sibling::*) then
                                        latex:text($config, ., ("tei-imprint14"), ', ')
                                    else
                                        (),
                                    if (pubPlace) then
                                        latex:inline($config, ., ("tei-imprint15"), pubPlace)
                                    else
                                        (),
                                    if (following-sibling::biblScope[@unit='page']) then
                                        latex:text($config, ., ("tei-imprint16"), ': ')
                                    else
                                        (),
                                    if (publisher) then
                                        latex:inline($config, ., ("tei-imprint17"), publisher)
                                    else
                                        (),
                                    if (date) then
                                        latex:text($config, ., ("tei-imprint18"), ', ')
                                    else
                                        (),
                                    if (date) then
                                        latex:inline($config, ., ("tei-imprint19"), date)
                                    else
                                        (),
                                    if (biblScope[@unit='page']) then
                                        latex:text($config, ., ("tei-imprint20"), ': ')
                                    else
                                        (),
                                    if (biblScope[@unit='page']) then
                                        latex:inline($config, ., ("tei-imprint21"), biblScope[@unit='page'])
                                    else
                                        (),
                                    if (note) then
                                        latex:inline($config, ., ("tei-imprint22"), note)
                                    else
                                        (),
                                    if (not(ancestor::biblStruct//note)) then
                                        latex:inline($config, ., ("tei-imprint23"), '.')
                                    else
                                        ()
                                )
                ...
                case element(series) return
                    if (ancestor::biblStruct) then
                        latex:inline($config, ., ("tei-series1"), if (series) then
    (
        latex:text($config, ., ("tei-series2"), ', '),
        latex:text($config, ., ("tei-series3"), title[@level='s']),
        latex:text($config, ., ("tei-series4"), ', '),
        latex:text($config, ., ("tei-series5"), biblScope)
    )
