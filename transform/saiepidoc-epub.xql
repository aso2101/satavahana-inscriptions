...
module namespace model="http://www.tei-c.org/tei-simple/models/saiepidoc.odd/epub";
...
declare function model:apply($config as map(*), $input as node()*) {
                ...
                case element(div) return
                    if (ancestor::TEI[@type='general-biblio']) then
                        ...
                        if (@type='bibliography') then
                            (
                                html:heading($config, ., ("tei-div1"), 'Secondary bibliography', 3),
                                html:section($config, ., ("tei-div2", "bibliography-secondary"), listBibl)
                            )

                        else
                            if (not((@type='textpart') or (@type='apparatus'))) then
                                (
                                    html:heading($config, ., ("tei-div1", (@type)), concat(upper-case(substring(@type,1,1)),substring(@type,2)), 3),
                                    html:section($config, ., ("tei-div2", (@type)), .)
                                )

                            else
                                if (@type='apparatus') then
                                    (
                                        html:section($config, ., ("tei-div", (@type)), .)
                                    )
    ...
    html:text($config, ., ("tei-lb1"), ' ('),
    html:inline($config, ., ("tei-lb2"), if (@n) then @n else count(preceding-sibling::lb) + 1),
    html:text($config, ., ("tei-lb3"), ') ')
)
)
                case element(lg) return
                    if (@met or @n) then
                        ...
                        html:block($config, ., ("tei-lg", "block"), .)
                ...
                case element(revisionDesc) return
                    if ($parameters?teiHeader-type='epidoc') then
                        ...
                        (
                            html:block($config, ., ("tei-revisionDesc"), concat('Last modified on: ',change[position()=last()]/@when))
                        )
    ...
    (
        html:text($config, ., ("tei-author1"), surname),
        html:text($config, ., ("tei-author2"), ', '),
        html:text($config, ., ("tei-author3"), forename)
    )

else
    if (name) then
        html:text($config, ., ("tei-author"), name)
    else
        $config?apply($config, ./node()))
                        else
                            $config?apply($config, ./node())
                ...
                case element(fileDesc) return
                    if ($parameters?header='short') then
                        ...
                        if (ancestor::TEI[@type='general-biblio']) then
                            html:paragraph($config, ., ("tei-fileDesc1"), sourceDesc)
                        else
                            if ($parameters?teiHeader-type='epidoc') then
                                ...
                                if ((@level='j' or @level='m')  and ancestor::biblStruct) then
                                    ...
                                    if (not(@level) and parent::bibl) then
                                        ...
                                        if (@type='short' and ancestor::biblStruct) then
                                            html:inline($config, ., ("tei-title5", "vedette"), .)
                                        else
                        ...
                        if (ancestor::biblStruct and @unit='volume') then
                            (
                                if (monogr/author or monogr/editor) then
                                    html:text($config, ., ("tei-biblScope1"), ', ')
                                else
                                    (),
                                if (@unit='volume') then
                                    html:inline($config, ., ("tei-biblScope2"), .)
                                else
                                    (),
                                if (@unit='volume') then
                ...
                case element(biblStruct) return
                    if (parent::listBibl) then
                        html:listItem($config, ., ("tei-biblStruct"), (
    html:block($config, ., ("tei-biblStruct"), (
    if (./@type) then
        html:inline($config, ., ("tei-biblStruct1"), @type)
    else
        (),
    if (./@xml:id) then
        html:inline($config, ., ("tei-biblStruct2"), @xml:id)
    else
        ()
)
),
    (
        html:block($config, ., ("tei-biblStruct"), if (.//title[@type='short']) then
    html:inline($config, ., ("tei-biblStruct"), .//title[@type='short'])
else
    (
        if (.//author/surname) then
            html:text($config, ., ("tei-biblStruct1"), .//author/surname)
        else
            if (.//author/name) then
                html:text($config, ., ("tei-biblStruct2"), .//author/name)
            else
                $config?apply($config, ./node()),
        html:text($config, ., ("tei-biblStruct1"), ' '),
        html:inline($config, ., ("tei-biblStruct2"), monogr/imprint/date)
    )
)
    )
,
    (
        if (@type='journalArticle' or @type='bookSection' or @type='encyclopediaArticle') then
            (
                html:inline($config, ., ("tei-biblStruct1"), analytic/author),
                html:text($config, ., ("tei-biblStruct2"), ', '),
                html:inline($config, ., ("tei-biblStruct3"), analytic/title[@level='a']),
                html:text($config, ., ("tei-biblStruct4"), ', '),
                if (@type='bookSection' or @type='encyclopediaArticle') then
                    (
                        if (@type='bookSection' or @type='encyclopediaArticle') then
                            html:text($config, ., ("tei-biblStruct1"), 'in ')
                        else
                            (),
                        html:inline($config, ., ("tei-biblStruct2"), monogr/title[@level='m']),
                        html:text($config, ., ("tei-biblStruct3"), ', '),
                        if (monogr/author) then
                            html:text($config, ., ("tei-biblStruct4"), 'by ')
                        else
                            (),
                        html:inline($config, ., ("tei-biblStruct5"), monogr/author),
                        if (monogr/editor) then
                            html:text($config, ., ("tei-biblStruct6"), 'edited by ')
                        else
                            (),
                        html:inline($config, ., ("tei-biblStruct7"), monogr/editor)
                    )
                ...
                if (@type='journalArticle') then
                    (
                        html:inline($config, ., ("tei-biblStruct1"), monogr/title[@level='j']),
                        html:text($config, ., ("tei-biblStruct2"), ', ')
                    )
                    ...
                    (),
                if (.//series) then
                    html:inline($config, ., ("tei-biblStruct5"), series)
                else
                    (),
                if (.//monogr/imprint) then
                    html:inline($config, ., ("tei-biblStruct6"), monogr/imprint)
                else
                    (),
                if (.//note) then
                    (
                        if (./note) then
                            html:inline($config, ., ("tei-biblStruct1"), ./note)
                        else
                            (),
                        if (.//imprint/note) then
                            html:inline($config, ., ("tei-biblStruct2"), ./note)
                        else
                            (),
                        if (not(ends-with(.//note,'.'))) then
                            html:inline($config, ., ("tei-biblStruct3"), '.')
                        else
                            ()
                    )
            ...
            if (@type='book') then
                (
                    html:inline($config, ., ("tei-biblStruct1"), monogr/author),
                    html:text($config, ., ("tei-biblStruct2"), ', '),
                    html:inline($config, ., ("tei-biblStruct3"), monogr/title[@level='m']),
                    html:text($config, ., ("tei-biblStruct4"), ', '),
                    if (monogr/imprint) then
                        (
                            html:inline($config, ., ("tei-biblStruct"), monogr/imprint)
                        )
                    ...
                    if (note) then
                        (
                            html:inline($config, ., ("tei-biblStruct1"), note),
                            if (not(ends-with(note,'.'))) then
                                html:inline($config, ., ("tei-biblStruct2"), '.')
                            else
                                ()
                        )
    ...
    (
        html:text($config, ., ("tei-editor1"), surname),
        html:text($config, ., ("tei-editor2"), ', '),
        html:text($config, ., ("tei-editor3"), forename)
    )

else
    if (name) then
        html:text($config, ., ("tei-editor"), name)
    else
        $config?apply($config, ./node()))
                        else
                            $config?apply($config, ./node())
                        ...
                        if (ancestor::biblStruct[@type='journalArticle']) then
                            (
                                if (biblScope[@unit='volume']) then
                                    html:inline($config, ., ("tei-imprint1"), biblScope[@unit='volume'])
                                else
                                    (),
                                if (biblScope[@unit='issue']) then
                                    html:inline($config, ., ("tei-imprint2"), biblScope[@unit='issue'])
                                else
                                    (),
                                if (date) then
                                    html:inline($config, ., ("tei-imprint3"), date)
                                else
                                    (),
                                if (../biblScope[@unit='page']) then
                                    html:text($config, ., ("tei-imprint4"), ': ')
                                else
                                    (),
                                if (note) then
                                    html:inline($config, ., ("tei-imprint5"), note)
                            ...
                            if (ancestor::biblStruct[@type='encyclopediaArticle'] or ancestor::biblStruct[@type='bookSection']) then
                                (
                                    ...
                                    if (following-sibling::*) then
                                        html:text($config, ., ("tei-imprint2"), ', ')
                                    else
                                        (),
                                    if (pubPlace) then
                                        html:inline($config, ., ("tei-imprint3"), pubPlace)
                                    else
                                        (),
                                    if (following-sibling::biblScope[@unit='page']) then
                                        ...
                                        (),
                                    if (publisher) then
                                        html:inline($config, ., ("tei-imprint5"), publisher)
                                    else
                                        (),
                                    if (date) then
                                        html:text($config, ., ("tei-imprint6"), ', ')
                                    else
                                        (),
                                    if (date) then
                                        html:inline($config, ., ("tei-imprint7"), date)
                                    else
                                        (),
                                    if (biblScope[@unit='page']) then
                                        html:text($config, ., ("tei-imprint8"), ': ')
                                    else
                                        (),
                                    if (biblScope[@unit='page']) then
                                        html:inline($config, ., ("tei-imprint9"), biblScope[@unit='page'])
                                    else
                                        (),
                                    if (note) then
                                        html:inline($config, ., ("tei-imprint10"), note)
                                    else
                                        (),
                                    if (not(ancestor::biblStruct//note)) then
                                        html:inline($config, ., ("tei-imprint11"), '.')
                                    else
                                        ()
                                )
                ...
                case element(series) return
                    if (ancestor::biblStruct) then
                        html:inline($config, ., ("tei-series"), if (series) then
    (
        html:text($config, ., ("tei-series1"), ', '),
        html:text($config, ., ("tei-series2"), title[@level='s']),
        html:text($config, ., ("tei-series3"), ', '),
        html:text($config, ., ("tei-series4"), biblScope)
    )
