...
module namespace model="http://www.tei-c.org/pm/models/hisoma-epigraphy/epub";
...
declare function model:apply($config as map(*), $input as node()*) {
            ...
            typeswitch(.)
                ...
                case element(ab) return
                    if (ancestor::div[@type='edition'] and $parameters?break='XML') then
                        ...
                        epub:block($config, ., ("tei-ab2"), .)
                case element(ab) return
                    epub:block($config, ., ("tei-ab"), .)
    ...
    html:text($config, ., ("tei-lb6"), ') ')
                ...
                case element(lg) return
                    if (@met or @n) then
                        (
                            ...
                            epub:block($config, ., ("tei-lg2", "verse-meter"), @met),
                            html:inline($config, ., ("tei-lg3", "verse-block"), .)
                        )

                    else
                        epub:block($config, ., ("tei-lg4", "block"), .)
                ...
                case element(revisionDesc) return
                    if ($parameters?teiHeader-type='epidoc') then
                        ...
                        (
                            epub:block($config, ., ("tei-revisionDesc1"), concat('Last modified on: ',change[position()=last()]/@when))
                        )
    ...
    epub:block($config, ., ("tei-biblStruct2"), (
    if (./@type) then
    ...
    (
        epub:block($config, ., ("tei-biblStruct5"), if (.//title[@type='short']) then
